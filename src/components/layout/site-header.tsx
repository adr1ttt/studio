...
export function SiteHeader() {
  ...
  return (
    <header 
      className={cn(
        "fixed top-0 left-0 right-0 z-[120] transition-all duration-500 ease-in-out",
        isVisible ? "translate-y-0" : "-translate-y-full"
      )}
    >
      {/* Top Bar News Ticker */}
      <div className="bg-background/95 backdrop-blur-xl border-b border-white/5 relative z-20">
        <NewsTicker items={newsItems} speed={30} />
      </div>

      <div className={cn(
        "transition-all duration-500 border-b",
        isScrolled 
          ? "bg-background/80 backdrop-blur-3xl border-white/10 py-0 shadow-[0_10px_30px_rgba(0,0,0,0.4)]" 
          : "bg-background/40 backdrop-blur-xl border-white/5 py-1"
      )}>
        <div className={cn(
          "container mx-auto px-4 flex items-center justify-between gap-4 transition-all duration-500",
          isScrolled ? "h-20 md:h-22" : "h-24 md:h-26"
        )}>
